...
class CMS_Breadcrumb
{
    ...
    function __construct( $args = array() )
    {
        $args = wp_parse_args( $args, array(
            'home_label'            => esc_html__( 'Home', 'soapee' ),
            '404_label'             => esc_html__( 'Not Found', 'soapee' ),
            'search_results_label'  => esc_html__( 'Search Results', 'soapee' ),
    ...
    function add_post_type_archive_entry()
    {
        ...
        if($post_type && $post_type->name === 'cms-service') {
            $label = esc_html__('Our Services','soapee');
        } else {
            $label = !empty($post_type) ? $post_type->labels->name : esc_html__('No post found!','soapee');
        }
        if ( $post_type )
        {
            $this->add_entry( $label, get_post_type_archive_link( get_post_type() ) );
        }
    }
    ...
    function add_tag_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Tag:', 'soapee' ).' '.single_tag_title( '', false ) ).' '.get_tag_link( $queried_object->term_id
        );
    }
    ...
    function add_author_entry()
    {
        ...
        $this->add_entry(
            esc_html__( 'Author:', 'soapee' ).' '.$userdata->display_name
        );
    }
    ...
    function add_tax_entry()
    {
         ...
         */
        $entries = apply_filters( 'soapee_breadcrumb_taxonomy', $entries, $this_term );
    ...
    function add_paged_entry()
    {
        ...
        {
            $this->add_entry(
                apply_filters( 'soapee_breadcrumb_paged', sprintf( 'Page %s', $page ) ),
                ''
            );
        }
    }
    ...
    function add_single_ancestor_entry( $post )
    {
        ...
        if ( $post_object )
        {
            ...
            if($post_object->name === 'cms-service') {
                $label = esc_html__('Our Services','soapee');
            } else {
         ...
         */
        $entries = apply_filters( 'soapee_breadcrumb_single', $entries, $post );
 ...
 */
if(!function_exists('soapee_breadcrumb')){
    function soapee_breadcrumb($args = [])
    {
        ...
        foreach ( $entries as $entry )
        {
            ...
            {
                printf(
                    '<a class="breadcrumb-entry" href="%1$s">%2$s</a>%3$s',
                    esc_url( $entry['url'] ),
