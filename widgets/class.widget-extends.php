 ...
 *
 * @author  Chinh Duong Manh (duongmanhchinh@gmai.com)
 * @version 1.0
 * @package Soapee
 * @since   Soapee 1.0
 */

class Soapee_Widget_Extends
{
    /**
     * Construction
     */
    ...
    {
        if ( ! isset( $instance['el_wrap_class'] ) )
        {
            $instance['el_wrap_class'] = '';
        }
        ...
        <p>
            ...
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-use_theme_style"><?php esc_html_e( 'Use Theme Style', 'soapee' ); ?></label>
        </p>
        <?php
        }
        ...
        ?>
        <p>
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-el_class"><?php esc_html_e( 'CSS Class', 'soapee' ); ?></label>
        ...
        <p>
            ...
            <label for="widget-<?php echo esc_attr( $widget->id_base . '-' . $widget->number ); ?>-hide_title"><?php esc_html_e( 'Hide widget title', 'soapee' ); ?></label>
        </p>
        <?php
...
new Soapee_Widget_Extends();
if(!function_exists('extend_widget_title')){
    add_filter('widget_title', 'extend_widget_title', 10, 2);
    function extend_widget_title($title, $instance){
        if(isset($instance['title']) && $instance['title'] === 'RSS') return $title;
        ob_start();
        $title = soapee_cut_join_string_by_separator($title);
        return ob_get_clean();
    }
}